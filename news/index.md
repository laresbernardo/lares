...
- feat: new encrypted family of functions encrypt_file(),
  read_encrypted(), write_encrypted(), hex_to_raw(), raw_to_hex()
- fix: get_creds() and cache returns objects correctly
- fix: improved order of messages in dont_sleep() + allow single value
  for sec_range
...
- fix: prepare compatibility with ggplot2 4.0.0
  [\#59](https://github.com/laresbernardo/lares/issues/59)
- feat: improve robyn_modelselector()’s weights based on experience,
  mainly confidence criteria
...
- docs: explicitly defined minimum versions of imported and suggested
  packages
...
- fix: mCPA calculation should be inverted (1 / provided marginal which
  is initResponseMargUnit)
- fix: marginal results on robyn_performance() when OutputCollect has
  more than one model
...
- fix: rename outlier_tukey() and allow for turkey too
  [\#58](https://github.com/laresbernardo/lares/issues/58)

## lares 5.2.11 (2025-01-09)

CRAN release: 2025-01-09

- feat: new “certainty” and “cluster_sd” metrics for
  robyn_modelselector() with spend_wt lazy boolean param
- feat: new scale argument for normalize() to customize minimum and
  maximum (instead of default 0, 1 range)
- feat: new non_promo param for robyn_performance() to include
...
- feat: new cross models budget allocation functions robyn_xchannels()
  and robyn_xchannels()
  [\#57](https://github.com/laresbernardo/lares/issues/57)
- feat: formatNum() improvements to be more flexible with abbr = TRUE on
  very different scales of numbers
...
- recode: fix some codefactor warnings on return()’s + font warning in
  Windows [\#55](https://github.com/laresbernardo/lares/issues/55)

## lares 5.2.8 (2024-06-11)

CRAN release: 2024-06-11

- feat: allow corr_var() to pass string var input for loops - based on
  user feedback in StackOverflow
- feat: new quiet parameter for font check on theme_lares()
- feat: new baseline distance criteria for robyn_modelselector() and
  refactoring for better cleaner code
- feat: new robyn_performance() function to calculate performance and
  contribution per channel
- fix: keep organic vars to calculate non-zero betas in
  robyn_modelselector()
- fix: correct solID filtering when multiple models are available in
  OutputCollect
- docs: include example to recreate a Robyn model, added external posts,
  centralized some functions .md

## lares 5.2.7 (2024-04-22)

CRAN release: 2024-04-22

- feat: enable font_dirs parameter and multiple fonts to check in
  font_exists()
- feat: sort clusters in plot by importance in robyn_modelselector()
- fix: when tied ranks on robyn_modelselector() set integer number of
  stars
- fix: faster dir_size() using du -hs instead + default path getwd() in
  dir_size()
...
- docs: removed h2o dependency and passed to Suggests to lighten the
  package
...
- feat: new maze solving functions inspired by micromouse competitions:
  maze_solve() & maze_gridsearch()
- feat: new gemini\_\*() functions to interact with Google’s Gemini API
- fix: inverted NRMSE, DECOMP.RSSD and MAPE + skip boost-performance
  option + non-clustered sols in robyn_modelselector()

## lares 5.2.4 (2023-12-07)

CRAN release: 2023-12-07

- feat: new robyn_modelselector() to help users visually select Robyn
  models based on several metrics available and analysts weights
  criteria
- feat: new robyn_hypsbuilder() to automatically build hyperparameters
  list for paid media and organic variables, ready to be used with Robyn
...
- feat: improve font_exists() suggestions and new use-case to search for
  all available fonts
- feat: additional parameter for gpt\_\*() functions: num_retries,
  temperature, max_tokens, pause_base
...
- feat: New ‘quiet’ and ‘include_regions’ parameters to allow additional
  information on holidays
- fix: better handling of scrapping responses using header and more
  flexible date formats for holidays()
  [\#45](https://github.com/laresbernardo/lares/issues/45)
- fix: better gpt_table() prompt to deal with numeric values and
  delimiters
...
- docs: fix dependent vs independent term across parameters
  [\#51](https://github.com/laresbernardo/lares/issues/51)
- docs: removed rdrop2 recommended dependency given it was deprecated
  from CRAN
...
- fix: when no variables importance is returned by h2o
  [\#43](https://github.com/laresbernardo/lares/issues/43)
...
- feat: new gpt\_\*() function to classify, tag, extract, format,
  convert, table, translate using ChatGPT
- feat: enable repo parameter for get_mp3() so when the main repo is
  down
...
- feat: centralized all FB API results through fb_process() to
  standardize results, pagination and partial results if API crashes
  while paging
- feat: upgrade FB API default version from v13.0 to v16.0 (latest)
- feat: new filtering parameter for fb_insights()
- feat: new fb_report_check() function to check async reports status
  with a live functionality
...
- deprecated: fb_post() and fb_posts() given their deprecation since
  v2.4
- recode: filled statusbar() fixed length as default
- recode: conditional expressions with scalar logical operators (\|\|,
  &&) and avoid sapply \[codefactor.io\]

## lares 5.1.4 (2022-09-08)

CRAN release: 2022-09-08

- fix: broken Wordle dictionary link from nytimes (crashed CRAN’s
  checks)
...
- feat: enabled automatic clustering number based on WSS variance using
  wss_var parameter in clusterKmeans()
- feat: new warnifnot() function, similar to stopifnot()
- feat: new exclude_here param enables multiple options for same
  position in scrabble_words()(Wordle adaptation)
...
- docs: reduced dependencies by removing magrittr (Imports), DBI,
  forecats, plotly, rtweet, tm, and wordcloud (Suggests)
...
- docs: explicitly pick exported tidyverse function to be exported,
  instead of all
...
- fix: issue with cor.test = “exact” default value on corr\*() functions
  [\#34](https://github.com/laresbernardo/lares/issues/34)
  [@michellekee](https://github.com/michellekee)
...
- docs: Excluded xml2 dependency temporally to avoid rvest (\>= 1.0.0).
  Used httr instead. Added rpart.plot as well.
...
- feat: New reduce_pca() and reduce_tsne() functions reduction
  dimentionality
...
- fix: specified R and rvest min versions +
  [@ledell](https://github.com/ledell) recommendations
- docs: use on problematic examples and reduces dependencies: mice,
  rlist, skimr, sp
