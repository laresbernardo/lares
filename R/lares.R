...
cran_logs <- function(input = "lares",
  ...
  if (is.vector(input)) {
    ...
    cran.df <- bind_rows(scrap) %>%
      mutate(date = unlist(lapply(.data$downloads, function(x) x[[1]])),
  ...
  if (nrow(input) >= length(package) * 2 & plot == TRUE) {
    ...
    plot <- ggplot(input, aes(x=.data$date)) +
      ...
      labs(title = glued("CRAN: {x} downloads", x = v2t(package, and = "and")),
           subtitle = sprintf("(%s to %s)", min(input$date), max(input$date)),
           x = "Downloaded Date", y = "Cumulative Downloads",
           color = NULL) + 
      facet_grid(.data$package~., scales = "free") +
      geom_line(aes(y = .data$MN), linetype = "dashed") +
      geom_text(data = dMean, aes(x = .data$date, y = .data$MN, label = .data$MN_label),
                vjust = -0.5, size = 2.5) +
      scale_y_abbr() + expand_limits(y = 0)
  }
