...
#' @param thres Numeric. Z-Score threshold for n standard deviations.
#' @param mad Boolean. Use median absolute deviation instead?
#' @export
outlier_zscore <- function(x, thres = 3, mad = FALSE) {
  if (mad == FALSE)
    !abs(x - mean(x, na.rm = TRUE)) <= thres * sd(x, na.rm = TRUE)
  else
    !abs(x - median(x, na.rm = TRUE)) <= thres * mad(x, na.rm = TRUE)
}
...
#' are the lower and upper quartiles respectively, k - some nonnegative 
